...
function createScene() {
  ...
  const sphere = BABYLON.MeshBuilder.CreateSphere("sphere", { diameter: 20, segments: 68 }, scene);
  sphere.position.y = 1;
  const ground = BABYLON.MeshBuilder.CreateGround("ground", { width: 30, height: 36 }, scene);
  ...
  resetButton.addEventListener("click", () => {
    ...
    camera.setTarget(BABYLON.Vector3.Zero());
    statusText.textContent = "Camera reset to the starting view ok.";
  });
  return scene;
}
...
try {
  ...
  console.log("Week 4: sphere and ground scene loaded. Helo.");
